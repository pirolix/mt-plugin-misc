package MT::Plugin::OMV::Filter::Acronym;
...
my $plugin = __PACKAGE__->new ({
        ...
        doc_link => 'http://www.magicvox.net/archive/2011/01161530/',
        description => <<HTMLHEREDOC,
<__trans phrase="Convert acronyms to ACRONYM HTML tag along the dictionary.">
HTMLHEREDOC
...
sub _hdlr_abbr {
    ...
    $text =~ s!($src)!sub {
        if (defined $map{$1}) {
            my $out = qq{<acronym title="$map{$1}">$1</acronym>};
